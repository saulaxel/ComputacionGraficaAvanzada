...
glm::mat4 modelMatrixBuzz = glm::mat4(1.0f); // 1.0 = Diagonal principal unitaria

float rotDartHead = 0.0,
	  rotDartLeftArm = 0.0,
	  rotDartLeftHand = 0.0,
	  rotDartRightArm = 0.0,
	  rotDartRightHand = 0.0,
	  rotDartLeftLeg = 0.0,
	  rotDartRightLeg = 0.0;

float rotBuzzHead = 0.0,
      rotBuzzLeftArm = 0.0,
	  rotBuzzLeftForearm = 0.0,
      rotBuzzLeftHand = 0.0,
      rotBuzzRightArm = 0.0,
	  rotBuzzRightForearm = 0.0,
      rotBuzzRightHand = 0.0,
      rotBuzzLeftLeg = 0.0,
      rotBuzzRightLeg = 0.0;
...
void mouseCallback(GLFWwindow *window, double xpos, double ypos) {
	int xposi = (int)xpos;
	int yposi = (int)ypos;
	offsetX = xposi - lastMousePosX;
	offsetY = yposi - lastMousePosY;
	lastMousePosX = xposi;
	lastMousePosY = yposi;
}
...
bool processInput(bool continueApplication) {
	...
	if (modelSelected == 4 && glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)
		modelMatrixBuzz = glm::translate(modelMatrixBuzz, glm::vec3(0.0f, 0.0f, +0.02f));
...
void applicationLoop() {
	...
	modelMatrixBuzz = glm::translate(modelMatrixBuzz, glm::vec3(3.0, 0.0, -5));

	// Variables to interpolation key frames dart
	fileName = "../animaciones/animation_dart_joints.txt";
	keyFramesDartJoints = getKeyRotFrames(fileName);
	keyFramesDart = getKeyFrames("../animaciones/animation_dart.txt");

	// Variables to interpate key frames buzz
	...
	while (psi) {
		...
		if (currTime - lastTime < 1 / FPS_DESEADOS) {
			glfwPollEvents();
			continue;
		}
		lastTime = currTime;
		TimeManager::Instance().CalculateFrameRate(true);
		deltaTime = (float)TimeManager::Instance().DeltaTime;
		...
		// Renderizamos a buzz
		glm::mat4 modelMatrixBuzzBody = glm::scale(modelMatrixBuzz, glm::vec3(2.5f, 2.5f, 2.5f));

		modelBuzzTorso.render(modelMatrixBuzzBody);

		glm::mat4 modelMatrixBuzzLeftArm = glm::mat4(modelMatrixBuzzBody);
		modelMatrixBuzzLeftArm = glm::translate(modelMatrixBuzzLeftArm, glm::vec3(+0.173194f, +0.577878f, -0.019591f));
		...
		// Lógica para guardar los keyframes
		if (record && modelSelected == 1) {
			...
			if (indexFrameDartJointsNext > (int)keyFramesDartJoints.size() - 1) {
		...
		if (record && modelSelected == 2) {
			...
			if (indexFrameDartNext > (int)keyFramesDart.size() - 1)
		...
		case 1:
			...
			break;
		case 2: {
			float immediateRot = 1.0f;
			modelMatrixEclipse = glm::translate(modelMatrixEclipse, glm::vec3(0.0f, 0.0f, 0.025f));
			modelMatrixEclipse = glm::rotate(modelMatrixEclipse, glm::radians(immediateRot), glm::vec3(0.0f, 1.0f, 0.0f));
			rotCount += immediateRot;
			rotWheelsX += 0.05f;
			rotWheelsY += 0.02f;
			if (rotWheelsY > 0.24f) {
				rotWheelsY = 0.24f;
		...
		// Máquina de estado del lambo car
		switch (stateDoor) {
		case 0:
			dorRotCount += 0.5f;
		...
		default:
			std::cerr << "Esto no tendría que pasar" << std::endl;
			break;
		}
