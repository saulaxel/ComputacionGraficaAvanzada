...
constexpr float terrainBaseSize = 300;
constexpr float terrainPosX = 100, terrainPosY = 0, terrainPosZ = 100;
Terrain terrain(-1, -1, terrainBaseSize, 16, "tex/height-map.png");

GLuint textureWindowID, textureHighwayID, textureLandingPadID;
GLuint textureTerrainBackgroundID, textureTerrainRID, textureTerrainGID, textureTerrainBID, textureTerrainBlendMapID;
GLuint textureParticleFountainID, textureParticleFireID, fireTexID;
GLuint skyboxTextureID;

GLenum skyboxTypes[6] = {
...
std::vector<float> lamp1Orientation = { -17.0f, -82.67f, 23.70f };
std::vector<glm::vec3> lamp2Position = { glm::vec3(-36.52f, 0, -23.24f),
		glm::vec3(-52.73f, 0, -3.90f) };
std::vector<float> lamp2Orientation = {21.37f + 90, -65.0f + 90};

// Blending model unsorted
std::map<std::string, glm::vec3> blendingUnsorted = {
		{"aircraft", glm::vec3(10.0f, 0.0f, -17.5f)},
		{"lambo", glm::vec3(23.0f, 0.0f, 0.0f)},
		{"heli", glm::vec3(5.0f, 10.0f, -5.0f)},
		{"fountain", glm::vec3(5.0f, 0.0f, -40.0f)},
		{"fire", glm::vec3(0.0f, 0.0f, 7.0f)}
};
...
float currTimeParticlesAnimationFire, lastTimeParticlesAnimationFire;
...
void configurarSkybox();
GLuint initFireTexture();
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	terrain.setPosition(glm::vec3(terrainPosX, terrainPosY, terrainPosZ));
	...
	// Definiendo la textura de ventana
	textureWindowID = crearTextura(
		/* filename = */ "../Textures/ventana.png",
		/* num_textures = */ 1,
		/* wrap_s = */ GL_REPEAT,
		/* wrap_t = */ GL_REPEAT,
	...
	// Definiendo la textura a de calle
	textureHighwayID = crearTextura(
		/* filename = */ "../Textures/highway.jpg",
		/* num_textures = */ 1,
		/* wrap_s = */ GL_REPEAT,
		/* wrap_t = */ GL_REPEAT,
		/* min_filter = */ GL_LINEAR,
	...
	// Definiendo la textura del landingPad
	textureLandingPadID = crearTextura(
		/* filename = */ "../Textures/landingPad.jpg",
		/* num_textures = */ 1,
		/* wrap_s = */ GL_REPEAT,
		/* wrap_t = */ GL_REPEAT,
		/* min_filter = */ GL_LINEAR,
	...
	// Definiendo la textura del fondo del terreno
	textureTerrainBackgroundID = crearTextura(
		/* filename = */ "tex/arena.jpg",
		/* num_textures = */ 1,
		/* wrap_s = */ GL_REPEAT,
		/* wrap_t = */ GL_REPEAT,
		/* min_filter = */ GL_LINEAR,
	...
	// Textura del lodo en el terreno
	textureTerrainRID = crearTextura(
		/* filename = */ "tex/pista.jpg",
		/* num_textures = */ 1,
		/* wrap_s = */ GL_REPEAT,
		/* wrap_t = */ GL_REPEAT,
		/* min_filter = */ GL_LINEAR,
	...
	// Textura de flores en el terreno
	textureTerrainGID = crearTextura(
		/* filename = */ "tex/cesped.jpg",
		/* num_textures = */ 1,
		/* wrap_s = */ GL_REPEAT,
		/* wrap_t = */ GL_REPEAT,
		/* min_filter = */ GL_LINEAR,
	...
	// Textura del camino
	textureTerrainBID = crearTextura(
		/* filename = */ "tex/lineas-blancas.jpg",
		/* num_textures = */ 1,
		/* wrap_s = */ GL_REPEAT,
		/* wrap_t = */ GL_REPEAT,
		/* min_filter = */ GL_LINEAR,
	...
	// Textura del mapa de mezclas
	textureTerrainBlendMapID = crearTextura(
		/* filename = */ "tex/mapa-mezcla-circuito.png",
		/* num_textures = */ 1,
		/* wrap_s = */ GL_REPEAT,
		/* wrap_t = */ GL_REPEAT,
		/* min_filter = */ GL_LINEAR,
	...
	// Textura para las partículas
	textureParticleFountainID = crearTextura(
		/* filename = */ "../Textures/bluewater.png",
		/* num_textures = */ 1,
		/* wrap_s = */ GL_REPEAT,
		/* wrap_t = */ GL_REPEAT,
		/* min_filter = */ GL_LINEAR,
	...
	// Textura del fuego
	textureParticleFireID = crearTextura(
		/* filename = */ "../Textures/fire.png",
		/* num_textures = */ 1,
		/* wrap_s = */ GL_REPEAT,
		/* wrap_t = */ GL_REPEAT,
		/* min_filter = */ GL_LINEAR,
	...
	// Inicializar una textura para el fuego
	initFireTexture();
	
	// Parámetros del shader para el fuego
...
void mouseCallback(GLFWwindow *window, double xpos, double ypos) {
	offsetX = static_cast<int>(xpos - lastMousePosX);
	offsetY = static_cast<int>(ypos - lastMousePosY);
	lastMousePosX = static_cast<int>(xpos);
	lastMousePosY = static_cast<int>(ypos);
}

void scrollCallback(GLFWwindow* window, double xoffset, double yoffset){
	distanceFromTarget -= static_cast<float>(yoffset);
	camera->setDistanceFromTarget(distanceFromTarget);
}
...
bool processInput(bool continueApplication) {
	...
	if(glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS)
		camera->mouseMoveCamera(offsetX, 0.0f, static_cast<float>(deltaTime));
	if(glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS)
		camera->mouseMoveCamera(0.0f, offsetY, static_cast<float>(deltaTime));
	offsetX = 0;
	offsetY = 0;
	...
	if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_RELEASE &&
		...
		rotDartLeftArm += 0.02f;
	else if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS &&
			glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS)
		rotDartLeftArm -= 0.02f;
	if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_RELEASE &&
			glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS)
		rotDartRightArm += 0.02f;
	else if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS &&
			glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS)
		rotDartRightArm -= 0.02f;
	if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_RELEASE &&
			glfwGetKey(window, GLFW_KEY_4) == GLFW_PRESS)
		rotDartLeftHand += 0.02f;
	else if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS &&
			glfwGetKey(window, GLFW_KEY_4) == GLFW_PRESS)
		rotDartLeftHand -= 0.02f;
	if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_RELEASE &&
			glfwGetKey(window, GLFW_KEY_5) == GLFW_PRESS)
		rotDartRightHand += 0.02f;
	else if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS &&
			glfwGetKey(window, GLFW_KEY_5) == GLFW_PRESS)
		rotDartRightHand -= 0.02f;
	if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_RELEASE &&
			glfwGetKey(window, GLFW_KEY_6) == GLFW_PRESS)
		rotDartLeftLeg += 0.02f;
	else if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS &&
			glfwGetKey(window, GLFW_KEY_6) == GLFW_PRESS)
		rotDartLeftLeg -= 0.02f;
	if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_RELEASE &&
			glfwGetKey(window, GLFW_KEY_7) == GLFW_PRESS)
		rotDartRightLeg += 0.02f;
	else if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS &&
			glfwGetKey(window, GLFW_KEY_7) == GLFW_PRESS)
		rotDartRightLeg -= 0.02f;
	...
	if (modelSelected == 2 && glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS){
		modelMatrixMayow = glm::rotate<float>(modelMatrixMayow, glm::radians(90.0f) * TimeManager::LastDeltaTime(), glm::vec3(0, 1, 0));
		animationIndex = 0;
	}else if (modelSelected == 2 && glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS){
		modelMatrixMayow = glm::rotate<float>(modelMatrixMayow, glm::radians(-90.0f) * TimeManager::LastDeltaTime(), glm::vec3(0, 1, 0));
		animationIndex = 0;
	}if (modelSelected == 2 && glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS){
		modelMatrixMayow = glm::translate(modelMatrixMayow, glm::vec3(0, 0, playerSpeed * TimeManager::LastDeltaTime()));
		animationIndex = 0;
	}else if (modelSelected == 2 && glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS){
		modelMatrixMayow = glm::translate(modelMatrixMayow, glm::vec3(0, 0, -playerSpeed * TimeManager::LastDeltaTime()));
		animationIndex = 0;
	...
	if(!isJump && keySpaceStatus){
		...
		tmv = 0;
		// sourcesPlay[3] = true;
		// Existe la función para parar el audio a la mala
		//alSourceStop(/* NO  se que vá aquí*/)
	}
...
void applicationLoop() {
	...
	modelMatrixFountain[3][1] = terrain.getHeightTerrain(modelMatrixFountain[3][0] , modelMatrixFountain[3][2]) + 0.2f;
	...
	while (psi) {
		...
		glm::mat4 lightProjection = glm::mat4(1.0f), lightView;
		...
		shaderMulLighting.setFloat("spotLights[0].linear", 0.074f);
		shaderMulLighting.setFloat("spotLights[0].quadratic", 0.03f);
		shaderMulLighting.setFloat("spotLights[0].cutOff", cosf(glm::radians(12.5f)));
		shaderMulLighting.setFloat("spotLights[0].outerCutOff", cosf(glm::radians(15.0f)));
		shaderTerrain.setVectorFloat3("spotLights[0].light.ambient", glm::value_ptr(glm::vec3(0.0f, 0.0f, 0.0f)));
		...
		shaderTerrain.setFloat("spotLights[0].linear", 0.074f);
		shaderTerrain.setFloat("spotLights[0].quadratic", 0.03f);
		...
		for (size_t i = 0; i < lamp1Position.size(); i++){
			glm::mat4 matrixAdjustLamp = glm::mat4(1.0f);
			matrixAdjustLamp = glm::translate(matrixAdjustLamp, lamp1Position[i]);
			matrixAdjustLamp = glm::rotate(matrixAdjustLamp, glm::radians(lamp1Orientation[i]), glm::vec3(0, 1, 0));
			matrixAdjustLamp = glm::scale(matrixAdjustLamp, glm::vec3(0.5f, 0.5f, 0.5f));
			matrixAdjustLamp = glm::translate(matrixAdjustLamp, glm::vec3(0, 10.3585f, 0));
			glm::vec3 lampPosition = glm::vec3(matrixAdjustLamp[3]);
			shaderMulLighting.setVectorFloat3("pointLights[" + std::to_string(i) + "].light.ambient", glm::value_ptr(glm::vec3(0.2f, 0.16f, 0.01f)));
			shaderMulLighting.setVectorFloat3("pointLights[" + std::to_string(i) + "].light.diffuse", glm::value_ptr(glm::vec3(0.4f, 0.32f, 0.02f)));
			shaderMulLighting.setVectorFloat3("pointLights[" + std::to_string(i) + "].light.specular", glm::value_ptr(glm::vec3(0.6f, 0.58f, 0.03f)));
			...
			shaderMulLighting.setFloat("pointLights[" + std::to_string(i) + "].quadratic", 0.01f);
			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(i) + "].light.ambient", glm::value_ptr(glm::vec3(0.2f, 0.16f, 0.01f)));
			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(i) + "].light.diffuse", glm::value_ptr(glm::vec3(0.4f, 0.32f, 0.02f)));
			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(i) + "].light.specular", glm::value_ptr(glm::vec3(0.6f, 0.58f, 0.03f)));
			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(i) + "].position", glm::value_ptr(lampPosition));
			shaderTerrain.setFloat("pointLights[" + std::to_string(i) + "].constant", 1.0f);
			shaderTerrain.setFloat("pointLights[" + std::to_string(i) + "].linear", 0.09f);
			shaderTerrain.setFloat("pointLights[" + std::to_string(i) + "].quadratic", 0.02f);
		}
		for (size_t i = 0; i < lamp2Position.size(); i++){
			...
			glm::vec3 lampPosition = glm::vec3(matrixAdjustLamp[3]);
			shaderMulLighting.setVectorFloat3("pointLights[" + std::to_string(lamp1Position.size() + i) + "].light.ambient", glm::value_ptr(glm::vec3(0.2f, 0.16f, 0.01f)));
			shaderMulLighting.setVectorFloat3("pointLights[" + std::to_string(lamp1Position.size() + i) + "].light.diffuse", glm::value_ptr(glm::vec3(0.4f, 0.32f, 0.02f)));
			shaderMulLighting.setVectorFloat3("pointLights[" + std::to_string(lamp1Position.size() + i) + "].light.specular", glm::value_ptr(glm::vec3(0.6f, 0.58f, 0.03f)));
			shaderMulLighting.setVectorFloat3("pointLights[" + std::to_string(lamp1Position.size() + i) + "].position", glm::value_ptr(lampPosition));
			shaderMulLighting.setFloat("pointLights[" + std::to_string(lamp1Position.size() + i) + "].constant", 1.0f);
			shaderMulLighting.setFloat("pointLights[" + std::to_string(lamp1Position.size() + i) + "].linear", 0.09f);
			shaderMulLighting.setFloat("pointLights[" + std::to_string(lamp1Position.size() + i) + "].quadratic", 0.01f);
			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(lamp1Position.size() + i) + "].light.ambient", glm::value_ptr(glm::vec3(0.2f, 0.16f, 0.01f)));
			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(lamp1Position.size() + i) + "].light.diffuse", glm::value_ptr(glm::vec3(0.4f, 0.32f, 0.02f)));
			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(lamp1Position.size() + i) + "].light.specular", glm::value_ptr(glm::vec3(0.6f, 0.58f, 0.03f)));
			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(lamp1Position.size() + i) + "].position", glm::value_ptr(lampPosition));
			shaderTerrain.setFloat("pointLights[" + std::to_string(lamp1Position.size() + i) + "].constant", 1.0f);
			shaderTerrain.setFloat("pointLights[" + std::to_string(lamp1Position.size() + i) + "].linear", 0.09f);
			shaderTerrain.setFloat("pointLights[" + std::to_string(lamp1Position.size() + i) + "].quadratic", 0.02f);
		}
		...
		if(record && modelSelected == 1){
			...
			if (indexFrameDartJointsNext > (int64_t)keyFramesDartJoints.size() - 1)
		...
		if (record && modelSelected == 2) {
			...
			if (indexFrameDartNext > (int64_t)keyFramesDart.size() - 1)
...
void renderScene(bool renderParticles){
	...
	modelCesped = glm::scale(modelCesped, glm::vec3(1, 1, 1));
	...
	for(std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++){
		if(it->second.first.compare("aircraft") == 0){
			...
			glActiveTexture(GL_TEXTURE1);
			glBindTexture(GL_TEXTURE_1D, fireTexID);
...
void configurarSkybox()
{
	...
	for (int i = 0; i < ARRAY_SIZE_IN_ELEMENTS(skyboxTypes); i++) {
		Texture skyboxTexture(skyboxFileNames[i]);
		FIBITMAP *bitmap = skyboxTexture.loadImage(true);
		unsigned char *data = skyboxTexture.convertToData(bitmap, imageWidth,
			imageHeight);
		if (data) {
			glTexImage2D(skyboxTypes[i], 0, GL_RGBA, imageWidth, imageHeight, 0,
...
/*
 * @param filename Archivo de imagen con la textura
 * @param apt_texturaID: Variable de salida. Se le asigna el ID de la textura generada
 * @param num_textures: Cantidad de texturas a generar
 * @param wrap_s, wrap_t: Especifican cómo se repite la textura
 * @param min_filter, mag_filter: Filtros
 */
GLuint crearTextura(
	std::string filename,
	GLsizei num_textures,
...
)
{
	...
	FIBITMAP* bitmap{ texture.loadImage() };
	// Convertimos el mapa de bits en un arreglo unidimensional de tipo unsigned char
	int imageWidth, imageHeight;
	unsigned char* data{
		// Esta función se encarga de inicializar imageWidth e imageHeight
		texture.convertToData(bitmap, imageWidth, imageHeight)
	};

	// Creando la textura con para el Cesped
	GLuint texturaID;
	glGenTextures(num_textures, &texturaID);

	// Enlazar esa textura a una tipo de textura de 2D.
	glBindTexture(GL_TEXTURE_2D, texturaID);

	// Establecer los parámetros de repetición de la textura
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrap_s);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrap_t);

	// Parámetros de filtrado de la textura
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, min_filter);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, mag_filter);

	// Verifica si se pudo abrir la textura
	if (data) {
		// Transferimoss los datos de la imagen a memoria
...
/*
 * Creando una textura a partir de números aleatorios distribuidos uniformemente
 */
GLuint initFireTexture()
{
